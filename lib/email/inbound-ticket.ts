...
export type PostmarkInboundWebhookPayload = {
  From: string;
  FromFull: {
    Email: string;
    Name: string;
...
export async function processInboundTicket(
  ...
  provider: "postmark" | "google_apps_script",
  mailboxThreadId?: string
) {
  const emailId = payload.MessageID;
  const inReplyTo = mailboxThreadId ?? payload.Headers?.find(h => h.Name.toLowerCase() === "in-reply-to")?.Value ?? null;
  const threadId = mailboxThreadId ?? inReplyTo ?? payload.MessageID;
  ...
  // A thread match only counts when this sender owns an open ticket. A reply
  // to an older closed thread must still find another open ticket for them.
  ...
  // Every other email from this sender goes to their newest open ticket,
  // regardless of subject or Gmail thread. No second ticket is created.
